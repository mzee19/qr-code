...
namespace App\Libraries\SimpleQrcode;

use App\Libraries\BaconQrCode\Common\ErrorCorrectionLevel;
use App\Libraries\BaconQrCode\Encoder\Encoder;
use App\Libraries\BaconQrCode\Exception\WriterException;
use App\Libraries\BaconQrCode\Renderer\Color\Alpha;
use App\Libraries\BaconQrCode\Renderer\Color\ColorInterface;
use App\Libraries\BaconQrCode\Renderer\Color\Rgb;
use App\Libraries\BaconQrCode\Renderer\Eye\EyeInterface;
use App\Libraries\BaconQrCode\Renderer\Image\EpsImageBackEnd;
use App\Libraries\BaconQrCode\Renderer\Image\ImageBackEndInterface;
use App\Libraries\BaconQrCode\Renderer\Image\ImagickImageBackEnd;
use App\Libraries\BaconQrCode\Renderer\Image\SvgImageBackEnd;
use App\Libraries\BaconQrCode\Renderer\ImageRenderer;
use App\Libraries\BaconQrCode\Renderer\Module\DotsModule;
use App\Libraries\BaconQrCode\Renderer\Module\GeneralShapeModule;
use App\Libraries\BaconQrCode\Renderer\Module\ModuleInterface;
use App\Libraries\BaconQrCode\Renderer\Module\RoundnessModule;
use App\Libraries\BaconQrCode\Renderer\RendererStyle\EyeFill;
use App\Libraries\BaconQrCode\Renderer\RendererStyle\Fill;
use App\Libraries\BaconQrCode\Renderer\RendererStyle\Gradient;
use App\Libraries\BaconQrCode\Renderer\RendererStyle\GradientType;
use App\Libraries\BaconQrCode\Renderer\RendererStyle\RendererStyle;
use App\Libraries\BaconQrCode\Writer;
use BadMethodCallException;
use InvalidArgumentException;
use App\Libraries\SimpleQrcode\DataTypes\DataTypeInterface;
//Add some shape, frame and eye.
use App\Libraries\BaconQrCode\Renderer\Eye\GeneralEye;
...
class Generator
{
    ...
    /**
     * The style to apply to the eye frame.
     *
     * @var string|null
     */
    protected $eyeFrame = null;
...
//        );
    ...
    public function eye(string $style): self
    {
        ...
        if (!in_array($style, $eyeStyles)) {
            $eyeStyleString = implode(", ",$eyeStyles);
            throw new InvalidArgumentException("style must be {$eyeStyleString}. {$style} is not a valid eye style.");
        }
    ...
    public function frame(string $frame): self
    {
        ...
        if (!in_array($frame,$frames)) {
            $frameString = implode(", ",$frames);
            throw new InvalidArgumentException("\$frame must be {$frameString}. {$frame} is not a valid eye frame.");
        }
    ...
    public function style(string $style, float $size = 0.5): self
    {
        ...
        if (!in_array($style, $styles)) {
            ...
            throw new InvalidArgumentException("Style must be {$styleString}. {$style} is not a valid.");
        }
    ...
    public function getModule(): ModuleInterface
    {
        ...
        return GeneralShapeModule::instance($this->styleSize,$this->style);
...
//        return SquareModule::instance();
    ...
    {
        return GeneralEye::instance($this->eyeStyle, $this->eyeFrame);
...
//        return new ModuleEye($this->getModule());
    ...
    public function getFill(): Fill
    {
        ...
        $eye1 = $this->eyeColors[0] ?? EyeFill::inherit();
        $eye2 = $this->eyeColors[0] ?? EyeFill::inherit();
    ...
    protected function formatClass(string $method): string
    {
        ...
        $class = "App\Libraries\SimpleQrcode\DataTypes\\".$method;

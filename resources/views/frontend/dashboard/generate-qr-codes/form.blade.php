                                    ...
                                    <form id="save-qr-code-form">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="name" id="general-name"
                                                   maxlength="20"
                                                   placeholder="{{__('e.g. vCard John Doe')}}" required onchange="updateBackHalf(true)"
                                                ...
                                                <select-campaign>
                                                    ...
                                                    <div class="input-group">
                                                        ...
                                                        >
                                                            ...
                                                            @php $campaignId = $action == 'Add' ? $campaign_id_without_hash ?? old('campaignId') : $generateQrCode->campaign_id @endphp
                                        ...
                                        <div class="col-auto">
                                            <div class="url">
                                                <i title="{{__('Short URL of dynamic QR code')}}"
                                                   id="ned-link"> {{($action == 'Add' ? url('/q/'.$parameters['unique_id']) : ($generateQrCode->ned_link ? $generateQrCode->ned_link : $generateQrCode->short_url))}}
                                                </i>
{{--                                                @if($action == 'Add' || !empty($generateQrCode->ned_link))--}}
                                                    <button class="btn btn-sm btn-icon" type="button"
                                                            data-toggle="modal"
                                                            data-target="#change-url-modal"><i class="fa fa-cog"></i>
                                                    </button>
{{--                                                @endif--}}
                                            </div>
                                        </div>
            ...
            <div class="fixed-right-qrbar">
                ...
                <div class="cardbox">
                    <div class="qrcode-container fgfgdf">
                        <div class="qrcode dashboard-generated-qr-code mt-3 img-contaner img-contane-color" id="generated-qr-code">
                            <div class="img-contaner">
                                <img class="qr-code-scan" src="{{checkImage(asset('storage/users/'.$generateQrCode->user_id.'/qr-codes/' . $generateQrCode->image),'default.svg',$generateQrCode->image)}}">
                            </div>
                        </div>
    ...
    <div class="modal fade url-change-modal" id="change-url-modal" >
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form method="post" enctype="multipart/form-data" id="change-url-form">
                    <div class="modal-header">
                        <h5 class="modal-title"
                            id="exampleModalLabel">{{__('Change URL')}}</h5>
                            ...
                            <div class="input-group">
                                ...
                                <div class="input-group mb-2 mr-sm-2">
                                    <div class="input-group-prepend">
                                        {{--                                        <select class="input-group-text" id="selected-domain">--}}
                                        {{--                                            @if(auth()->user()->userDomain->domain)--}}
                                        {{--                                            <option value="{{auth()->user()->userDomain->domain}}" checked>{{auth()->user()->userDomain->domain}}</option>--}}
                                        {{--                                            @endif--}}
                                        {{--                                            <option value="www.ned.link:8000">www.ned.link:8000</option>--}}
                                        {{--                                        </select>--}}
                                        <div class="input-group-text" id="selected-domain"></div>
                                    </div>
                                    <input type="text" class="form-control" id="back_half"
                                           placeholder="{{__('Url')}}"
                                           value="{{$action == 'Edit' ? (empty($generateQrCode->ned_link_back_half) ? $generateQrCode->unique_id : $generateQrCode->ned_link_back_half) : ''}}" name="backHalf" required>
                                </div>
                                <div class="w-100 text-center">
                                    <small><i>{{__('You can add your own custom domain from account.')}}</i></small>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        {{--                        <button type="button" class="btn btn-primary" onclick="updateUrl(this)">Change URL</button>--}}
                        <button type="button" class="btn btn-primary" onclick="updateBackHalf(this)">{{__('Save')}}</button>
                    </div>
                </form>
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        @foreach($logos as $index => $logo)
                                                                            @if(($action == 'Edit' && empty($generateQrCode->logo_image)) ? json_decode($generateQrCode->config, true)['logo'] == $logo->id : false)
                                                                                <script
...
{{--                                                                                {{json_decode($generateQrCode->config, true)['qrCodeFrameId'] == $qrCodeFrame->id}}--}}
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <img
                                                                            hidden=""
                                                                            id="image_cropper"
                                                                            src="{{$action == 'Add' ? '#' : ($generateQrCode->crop ? asset('storage/users/'.auth()->user()->id.'/qr-codes/transparent-images/'.$generateQrCode->transparent_background) : '#')}}">
                                                ...
                                                <div class="cardbox ">
                                                    ...
                                                    <div
                                                        ...
                                                        <div
                                                            ...
                                                            @endif
                                                            <p class="alerttxt bottom-text text-center mb-3 mt-3">
    ...
    {{--Campaign Warning--}}
    <div class="modal fade" id="campaign-warning-model">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{__('Campaign Warning')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    {{__('Are you sure to proceed without creating a New Campaign?')}}
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{__('No')}}</button>
                    <button class="btn btn-danger" title="{{__('Yes')}}"
                            onclick="checkCampaignValue(true)">{{__('Yes')}}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('js')
    <script>
        ...
        var qrCodeFrameId = '{{ ($action == 'Add') ? 0 : (isset(json_decode($generateQrCode->config, true)['qrCodeFrameId']) ? json_decode($generateQrCode->config, true)['qrCodeFrameId']  : '0')}}';
        ...
        var formData = '';
        var addCampaignWarningMessage = true;
        var campaignConfirmation = false;
        ...
        //Ned Link Creadential
        var nedLink = '{{($action == 'Add') ? null : ($generateQrCode->ned_link ?? '') }}';
        var backHalfId = '{{($action == 'Add') ? null : $generateQrCode->ned_link_back_half_id }}';
        var backHalf = '{{($action == 'Add') ? null : $generateQrCode->ned_link_back_half }}';
        // Get User Domain
        var nedLinkDomain = '';
        var nedLinkDomainWithHalfBackUrl = '';
        ...
        var defaultQrCodeFrameId = '{{ ($action == 'Add') ? 0 : (isset(json_decode($generateQrCode->config, true)['qrCodeFrameId'])? json_decode($generateQrCode->config, true)['qrCodeFrameId']: '')}}';
        ...
        var previousBackHalf = '{{$action == 'Edit' ? (empty($generateQrCode->ned_link_back_half) ? $generateQrCode->unique_id : $generateQrCode->ned_link_back_half) : ''}}';
        ...
        function makeQrCode() {
            var nedLinkCallApi = '{{$action == 'Add' ? true : false}}';
            if (editCallBackFirst) {
                var data = {
                    "config": {
                        "body": shapeName,
                ...
                formData.append('ned_link_call', nedLinkCallApi);
                // BackHalf change
                if (nedLinkDomain) {
                    formData.append('ned_link_domain_url', nedLinkDomainWithHalfBackUrl);
                }
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 1) {
                            if (firstImageAppend) {
                                ...
                                $('#ned-link').html(nedLink);
                                $('#ned-link-input').val(nedLink);

                                if (response.back_half) {
                                    $('#back_half').val(response.back_half);
                                    backHalfId = response.back_half_id;
                                    backHalf = response.back_half;
                                }
        ...
        function saveQrCode() {
            ...
            formData.append('back_half_id', backHalfId);
            formData.append('back_half', backHalf);
        ...
        $('#save-qr-code-btn').on('click', function () {
            ...
            var campaignDisplay = $('.form-new-campaign').css("display");
            var selectedCampaignId = $('select[name="campaignId"] option').filter(':selected').val()
            if (!addCampaignWarningMessage) {
                addCampaignWarningMessage = true
            }
            if (campaignConfirmation) {

            } else if (campaignDisplay != 'none' && selectedCampaignId == '') {
                checkCampaignValue(campaignConfirmation)
            }
            if ($('#save-qr-code-form').valid() && $('#dynamic-content-type').valid() && addCampaignWarningMessage == true) {
        ...
        function checkCampaignValue(confirm) {
            if (confirm == true) {
                addCampaignWarningMessage = true;
                campaignConfirmation = true;
                $("#save-qr-code-btn").trigger("click");
            } else {
                $('#campaign-warning-model').modal('show');
                addCampaignWarningMessage = false;
            }
        }
        ...
        function resetQrCode() {
            if (defaultCropperImageStatus == true) {
                ...
                if (defaultTransparentImageId) {
                    cropperImageStatus = false
                    $('#image_cropper').attr("src", '{{asset('storage/users/'.auth()->user()->id.'/qr-codes/transparent-images')}}' + '/' + defaultTransparentImageId);
                }
                $('#templates-tab').removeClass('active')
                $('#templates').removeClass('active')
                $('#templates').removeClass('show')
                ...
                qrCodeFrameId = defaultQrCodeFrameId;
        ...
        function updateUrl(ele) {
            ...
            $.ajax({
                ...
                url: "{{ route('frontend.user.qr-codes.change.url') }}",
                data: data,
                enctype: 'multipart/form-data',
                success: function (response) {
        ...
        function getCustomDomain() {
            var data = {
                ...
                'from': 'QRC',
                'generate_qr_code_id': '{{$generateQrCode->id}}',
            ...
            $.ajax({
                ...
                url: '{{route('frontend.user.listing.domain')}}',
                data: data,
                enctype: 'multipart/form-data',
                success: function (response) {
                    if (response.status == 1) {
                        $('#selected-domain').empty();
                        $('#selected-domain').text(response.domain);
                        nedLinkDomain = response.domain;
                    }
                }
            });
        }

        function updateBackHalf(ele) {
            if(dynamicContentField || (backHalfId != '')){
                if(ele != true){
                    frontendValidation('change-url-form');
                }

                var data = {
                    'customizeBackHalf': $('#back_half').val(),
                    '_id': backHalfId,
                    'title': $('#general-name').val(),
                    'from': 'QRC'
                }

                if(!dynamicContentFieldAction && typeof(ele[0]) != "undefined" && typeof(ele[0].id) != 'undefined'){
                    data['status'] = true;
                    data['customizeBackHalf'] = previousBackHalf;
                }

                if($('#change-url-form').valid()){
                    $.ajax({
                        type: "POST",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        ...
                        success: function (response) {
                            if (response.status == 1) {
                                if(ele != true) {
                                    nedLinkDomainWithHalfBackUrl = 'https://' + nedLinkDomain + '/' + $('#back_half').val();
                                    previousBackHalf = data['customizeBackHalf'];
                                    if(typeof(data['status']) == "undefined"){
                                        makeQrCode()
                                        setTimeout(function () {
                                            backHalf = $('#back_half').val();
                                            $('#ned-link').html(nedLinkDomainWithHalfBackUrl)
                                            $('#change-url-modal').modal('hide')
                                            if(backHalfId == ''){
                                                $('#uniqueId').val();
                                                $('#uniqueId').val(backHalf);
                                            }
                                        })
                                    }
                                }

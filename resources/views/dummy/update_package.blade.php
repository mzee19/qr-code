                                    ...
                                    <div class="tier-summary--panel">
                                        ...
                                        <div class="tier-summary-panel--header text-center">
                                            ...
                                            <span class="vat-prices d-block">
                                                ...
                                                <p class="monthly-price">{{config('constants.currency')['symbol']. ($vatAmount * $package->monthly_price + $package->monthly_price)}}
                                                    / <span class="tier-summary-month-descriptor">{{__('month')}}</span>
                                                </p>
                                                <p class="yearly-price">{{config('constants.currency')['symbol']. ($vatAmount * $package->yearly_price + $package->yearly_price)}}
                                                    / <span class="tier-summary-month-descriptor">{{__('Year')}}</span>
                                                </p>
                                            </div>
                                        </div>
                                        <div class="tier-summary-panel--body">
                                            <p class="tier-summary--description">{!! $package->description !!}</p>
{{--                                            <div class="short-details-holder">--}}
{{--                                                <ul class="tier-summary-consumables--list text-center">--}}
{{--                                                    <li>--}}
{{--                                                        <strong--}}
{{--                                                            class="feature-value d-block text-capitalize">1000</strong>--}}
{{--                                                        <span class="feature-name d-block text-uppercase">Links</span>--}}
{{--                                                    </li>--}}
{{--                                                    <li>--}}
{{--                                                        <strong--}}
{{--                                                            class="feature-value d-block text-capitalize">200</strong>--}}
{{--                                                        <span class="feature-name d-block text-uppercase">Custom Back-Half Links</span>--}}
{{--                                                    </li>--}}
{{--                                                </ul>--}}
{{--                                            </div>--}}
                                            <div class="bottom-btn-holder">
                                                ...
                                                @else
                                                    <button
                                                        class="btn btn-primary current-btn disabled {{$package->id== 2 ? 'd-none' : ''}}"
                                                        type="button" disabled="disabled"
                                                        id="1-{{$package->id}}-btn">{{__('Current')}}
                                                    </button>
                                                    <form action="{{route('frontend.user.subscribe')}}"
                                                          method="post" class="current-form"
                                                          id="1-{{$package->id}}-form">
                                                        @csrf
                                                        @method('GET')
                    ...
                    <div class="subscription-title--container">
                        <h2 class="text-center">
                            <span class="d-inline-block align-top">Detailed Comparison</span>
                        </h2>
                    </div>
                                                ...
                                                <li>
                                                    <p>{{translation($packageFeature->id,1,App::getLocale(),'name',$packageFeature->name)}}
                                                        <span data-toggle="tooltip" data-placement="top" title=""
                                                              data-original-title="{{translation($packageFeature->id,1,App::getLocale(),'name',$packageFeature->info)}}">
                                    ...
                                    <ul>
                                        ...
                                        <div class="tier-comparison-column--section">
                                            @foreach($packageFeatures as $packageFeature)
                                                ...
                                                @endphp
                                                    <div>
                                                        <li>{!!  isset($packageLinkFeature) ? (empty($packageLinkFeature->count) ? '<span><i class="fa fa-check-circle"></i></span>' : $packageLinkFeature->count) : '' !!}
                                                        </li>
                                                    </div>
                                            @endforeach
                                        </div>
        ...
        function setPackageType(type) {
            ...
            if (type == 'monthly') {
                ...
                $('.current-btn').each(function () {
                    var res = this.id.replace("1", "2")
                    $(this).attr('id', res);
                });

                $('.current-form').each(function () {
                    var res = $(this).attr('id').replace("1", "2")
                    $(this).attr('id', res);
                });
                ...
                $('.current-btn').each(function () {
                    var res = this.id.replace("2", "1")
                    $(this).attr('id', res);
                });

                $('.current-form').each(function () {
                    var res = $(this).attr('id').replace("2", "1")
                    $(this).attr('id', res);
                });
            }
        }
